...
export default function AdvertisingCarousel() {
  ...
  return (
    <div
      className="relative w-full overflow-hidden bg-gray-900 rounded-lg sm:rounded-xl md:rounded-2xl shadow-lg sm:shadow-xl md:shadow-2xl group"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    ...
    >
      {/* Main Carousel Container */}
      <div className="relative h-44 xs:h-44 sm:h-64 md:h-64 lg:h-80 xl:h-[22rem]">
        {advertisingSlides.map((slide, index) => {
          const isActive = index === currentSlide;
          const isPrev = index === (currentSlide - 1 + totalSlides) % totalSlides;
          const isNext = index === (currentSlide + 1) % totalSlides;
          ...
          return (
            ...
            >
              ...
              <div className="relative h-full max-w-7xl mx-auto px-3 xs:px-4 sm:px-6 lg:px-8">
                <div className="flex flex-col justify-center h-full max-w-xl md:max-w-2xl">
                  {/* Animated Content */}
                  <div
                    className={`transform transition-all duration-1000 delay-200 ${
                      isActive
                  ...
                  >
                    <h2 className="text-[10px] xs:text-xs sm:text-sm md:text-base font-semibold text-white/90 uppercase tracking-wider mb-1 xs:mb-1.5 sm:mb-2 md:mb-3">
                      {slide.subtitle}
                    </h2>
                    <h1 className="text-xl xs:text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-black text-white mb-2 xs:mb-2.5 sm:mb-3 md:mb-4 lg:mb-6 leading-tight drop-shadow-2xl">
                      {slide.title}
                    </h1>
                    <p className="text-xs xs:text-sm sm:text-base md:text-lg lg:text-xl text-white/90 mb-3 xs:mb-4 sm:mb-6 md:mb-8 lg:mb-10 font-medium max-w-xl line-clamp-2 sm:line-clamp-none">
                      {slide.description}
                    </p>
                    <button
                      className={`${slide.buttonColor} px-4 xs:px-5 sm:px-6 md:px-8 lg:px-10 py-1.5 xs:py-2 sm:py-2.5 md:py-3 lg:py-4 rounded-full font-bold text-xs xs:text-sm sm:text-base md:text-lg transition-all duration-300 transform hover:scale-105 active:scale-95 hover:shadow-xl sm:hover:shadow-2xl inline-flex items-center gap-1.5 sm:gap-2 group/btn`}
                    >
                      {slide.buttonText}
                      <ChevronRightIcon className="w-3 h-3 xs:w-3.5 xs:h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5 transition-transform group-hover/btn:translate-x-1" />
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="hidden sm:block absolute bottom-0 right-0 w-48 h-48 md:w-64 md:h-64 bg-white/5 rounded-full blur-3xl transform translate-x-24 md:translate-x-32 translate-y-24 md:translate-y-32"></div>
              <div className="hidden sm:block absolute top-0 left-0 w-64 h-64 md:w-96 md:h-96 bg-white/5 rounded-full blur-3xl transform -translate-x-32 md:-translate-x-48 -translate-y-32 md:-translate-y-48"></div>
            </div>
          );
        })}
      </div>
      ...
      <button
        onClick={prevSlide}
        className="hidden sm:flex absolute left-2 md:left-4 top-1/2 -translate-y-1/2 z-30 bg-white/20 hover:bg-white/40 backdrop-blur-md text-white p-2 md:p-3 rounded-full transition-all duration-300 opacity-0 group-hover:opacity-100 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-white/50 items-center justify-center"
        aria-label="Previous slide"
      >
      ...
      <button
        onClick={nextSlide}
        className="hidden sm:flex absolute right-2 md:right-4 top-1/2 -translate-y-1/2 z-30 bg-white/20 hover:bg-white/40 backdrop-blur-md text-white p-2 md:p-3 rounded-full transition-all duration-300 opacity-0 group-hover:opacity-100 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-white/50 items-center justify-center"
        aria-label="Next slide"
      >
      ...
      {/* Dot Indicators */}
      <div className="absolute bottom-2 xs:bottom-3 sm:bottom-4 md:bottom-6 left-1/2 -translate-x-1/2 z-30 flex gap-1.5 xs:gap-2 sm:gap-3">
        {advertisingSlides.map((_, index) => (
          <button
            key={index}
            onClick={() => goToSlide(index)}
            className={`group/dot transition-all duration-300 focus:outline-none ${
              index === currentSlide ? "w-6 xs:w-7 sm:w-8 md:w-10" : "w-1.5 xs:w-2 sm:w-2.5"
            }`}
            aria-label={`Go to slide ${index + 1}`}
          >
            <div
              className={`h-1.5 xs:h-2 sm:h-2.5 rounded-full transition-all duration-300 ${
                index === currentSlide
                  ? "bg-white shadow-md sm:shadow-lg sm:shadow-white/50"
                  : "bg-white/40 hover:bg-white/70 active:bg-white/80 group-hover/dot:scale-110"
              }`}
            ></div>
          </button>
        ))}
      ...
      <button
        onClick={() => setIsPaused(!isPaused)}
        className="hidden sm:flex absolute top-3 md:top-4 right-3 md:right-4 z-30 bg-white/20 hover:bg-white/40 backdrop-blur-md text-white p-2 rounded-full transition-all duration-300 opacity-0 group-hover:opacity-100 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-white/50 items-center justify-center"
        aria-label={isPaused ? "Play" : "Pause"}
      >
        {isPaused ? (
          <svg
